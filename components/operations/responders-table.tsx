...
import { AssignDutyDialog } from "./assign-duty-dialog"
...
export function RespondersTable({ responders }: RespondersTableProps) {
  ...
  const [assignDutyResponder, setAssignDutyResponder] = useState<Responder | null>(null)

  const filteredResponders = selectedTab === "All" ? responders : responders.filter((r) => r.status === selectedTab)
        ...
        <div className="flex gap-2">
          {["All", "Available", "Busy", "Off Duty"].map((tab) => (
            <Button
              ...
              className={cn(selectedTab === tab && "bg-orange hover:bg-orange/90")}
            ...
            <TableRow key={responder.id}>
              ...
              <TableCell>
                <div className="text-sm">
                  <p>Avg. Response Time: {responder.performanceMetrics.avgResponseTime}</p>
                  <p>Incidents Resolved: {responder.performanceMetrics.incidentsResolved}</p>
                </div>
              </TableCell>
              <TableCell>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => setAssignDutyResponder(responder)}>Assign Duty</DropdownMenuItem>
