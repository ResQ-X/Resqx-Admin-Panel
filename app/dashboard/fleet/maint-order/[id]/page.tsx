...
export default function MaintenanceOrderPage({
  ...
  useEffect(() => {
    const fetchMaintenanceOrder = async () => {
      ...
      try {
        ...
        console.error("Error fetching maintenance order:", err);
        setError("Failed to load maintenance order details");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (!order) return <div className="p-6">No maintenance order found.</div>;
      ...
      <div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-800">{order.maintenance_type}</span>
          </div>
        </div>
      </div>
